...
import type { Extra, CertificacionExtra } from '../../lib/types'

const IVA_RATE = 0.10

interface CertificacionDetalle extends CertificacionExtra {
  importe: number
}

export default function ExtraDetallePage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const { data: certExtras, error: certError } = await supabase
      ...
      for (const ce of (certExtras || [])) {
        const { data: certData, error: certError } = await supabase
          .from('certificaciones_extras')
          .select('*')
          .eq('id', ce.certificacion_extra_id)
          .single()
  ...
  if (error || !extra) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600">{error || 'Extra no encontrado'}</p>
        <Link href="/presupuesto-extras" className="text-blue-600 hover:underline mt-4 inline-block">
          Volver a Presupuesto de Extras
        </Link>
      </div>
    )
  }
  ...
  return (
    <div>
      <Link href="/presupuesto-extras" className="text-blue-600 hover:underline text-sm">
        ← Volver a Presupuesto de Extras
      </Link>

      <h2 className="text-2xl font-bold mt-4 mb-2">{extra.concepto}</h2>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 mt-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border border-zinc-200">
          <p className="text-sm text-zinc-500 mb-1">Importe</p>
      ...
      {certificaciones.length === 0 ? (
        <p className="text-zinc-500">No hay certificaciones para este extra.</p>
      ) : (
